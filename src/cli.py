...
def main():
    ...
    # trade-demo
    trade_parser = subparsers.add_parser('trade-demo', help='Run a trading demo (dry-run by default)')
    ...
    trade_parser.add_argument('--order-type', choices=['market', 'limit'], default='market', help='Order type')
    trade_parser.add_argument('--limit', type=float, default=None, help='Limit price (required for limit orders)')
    trade_parser.add_argument('--live', action='store_true', help='If provided, attempt to send a live order (requires --confirm)')
    trade_parser.add_argument('--confirm', action='store_true', help='Confirmation flag required to send live orders')
    ...
    if args.command == 'fetch':
        ...
        if demo_trade is None:
            print("Trading demo is not available. Ensure src/trading/ibkr.py is present and dependencies are installed.")
            return

        # Safety: require explicit confirm to send live orders
        if args.live and not args.confirm:
            print("To send a live order you must pass --live --confirm. By default this runs as a dry-run.")
            return

        is_dry = not args.live
        print(f"Running trading demo: symbol={args.symbol} qty={args.qty} order_type={args.order_type} dry_run={is_dry}")
        ...
        demo_trade(args.symbol, args.qty, dry_run=is_dry, order_type=args.order_type, limit_price=args.limit)
